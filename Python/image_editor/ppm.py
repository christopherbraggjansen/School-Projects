...
#Information on making an image negative found at
http://forums.codeguru.com/showthread.php?490165-Color-image-rgb-to-negative
# Information for flip horizontal from
http://stackoverflow.com/questions/3940128/how-can-i-reverse-a-list-in-python
#Information for rotate 90 degrees from
http://stackoverflow.com/questions/42519/how-do-you-rotate-a-two-dimensional-array and lots of help from Scott Heggen.
'''
##################################################################
import sys
...
'''
    ...
    def PPM_greyscale(self):
        ''''converts image to grey scale by replacing each color value for each pixel with the equivalent intensity value'''
        newpixellist=self.pixellist
        self.width=len(newpixellist[0])
        self.height=len(newpixellist)
        row=0
        for rowlist in newpixellist:
            col=0
            for pixel in rowlist:
                red=newpixellist[row][col][0]  # gets red value
                green=newpixellist[row][col][1]  # gets green value
                blue=newpixellist[row][col][2]  # gets blue value
                average=int((red+green+blue)/3)  # sets average of red green and blue channels
                newpixellist[row][col][0]=average # update red
                newpixellist[row][col][1]=average # update green
                newpixellist[row][col][2]=average # update blue
                col+=1
            row+=1
        print(self.outasciifile+ " output file turned grey scale")
        self.PPM_updatefrompixellist(newpixellist) # This call will update all member attributes appropriately.
    ...
    def PPM_rotateclockwise(self):
        ...
        self.width=len(self.pixellist[0])
        self.height=len(self.pixellist)
        listtoflip= []
        for g in range(self.width):
            temprow = []
            for h in range(self.height):
                temprow.append([0,0,0])
            listtoflip.append(temprow)

        rowindex = 0
        colindex = 0
        newwidth=len(listtoflip[0])
        for row in self.pixellist:
            colindex = 0
            for pixel in row:
                listtoflip[colindex][newwidth -1 - rowindex] = pixel #  -1 one is needed because newwidth is 1 higher than
                # number of pixels in row 1 of newpixel list this might be because python counted from 1 instead of 0
                colindex += 1
            rowindex += 1
        print(self.outasciifile+ " output file rotated.")
        self.PPM_updatefrompixellist(listtoflip)
    ...
    def PPM_negative(self):
        ...
        for rowlist in newpixellist:
            ...
            for pixel in rowlist:
                ...
                green=newpixellist[row][col][1]  # gets green value
                blue=newpixellist[row][col][2]  # gets blue value
                newpixellist[row][col][0]=255 - red  # update red
                newpixellist[row][col][1]=255 - green  # update green
                newpixellist[row][col][2]=255 - blue  # update blue
                col+=1
            row+=1
        print(self.outasciifile+ " output file turned negative.")
        self.PPM_updatefrompixellist(newpixellist) # This call will update all member attributes appropriately.

    def PPM_salt_pepper(self, noiselevel):
        '''
        This method adds noise to an image that looks like salt and pepper were spilled on the image.
        :return:
        '''
        newpixellist=self.pixellist
        self.width=len(newpixellist[0])
        ...
        for rowlist in newpixellist:
            col=0
            for pixel in rowlist:
                salt_pepper = random.randrange(0, noiselevel)
                pixels = pixel
                if salt_pepper == 0:
                    pixels = [0, 0, 0]
                elif salt_pepper == 1:
                    pixels = [255, 255, 255]
                newpixellist[row][col] = pixels
                col+=1
            row+=1
        print(self.outasciifile+ " noise added to output file.")
        self.PPM_updatefrompixellist(newpixellist) # This call will update all member attributes appropriately.
